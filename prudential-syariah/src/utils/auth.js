...
export const initDB = () => {
  if (!localStorage.getItem('users')) {
    const defaultUsers = [
      { name: 'Admin Utama', agentNumber: 'ADMIN-001', role: 'admin', target: 0 },
      { name: 'Ahmad Leader', agentNumber: 'LDR-001', role: 'leader', target: 2000 },
      { name: 'Siti Agent', agentNumber: 'AGT-001', role: 'agent', leaderId: 'LDR-001', target: 150 }
    ];
  ...
  if (!localStorage.getItem('folders')) localStorage.setItem('folders', JSON.stringify([{ id: 1, name: 'Selling', parentId: null }]));
  ...
  if (!localStorage.getItem('prospects')) {
    localStorage.setItem('prospects', JSON.stringify([
      { 
        id: 1, name: 'Bapak Ilham', phone: '08123456789', status: 'Contact', 
        appointmentDate: null, appointmentTime: [], followUpCount: 0, 
        followUpLogs: [], closingLogs: [], agentNumber: 'AGT-001', leaderId: 'LDR-001' 
      },
      { 
        id: 2, name: 'Ibu Ratna', phone: '08555666777', status: 'Closed', 
        appointmentDate: '2026-08-01', appointmentTime: ['10:00 - 11:00'], followUpCount: 0, 
        followUpLogs: [], 
        closingLogs: [{ id: 101, date: new Date().toISOString(), product: 'PRULink Syariah', amount: 25 }], 
        agentNumber: 'AGT-001', leaderId: 'LDR-001' 
      }
    ]));
  }
};
...
export const loginUser = (name, agentNumber) => {
  ...
  const user = users.find((u) => u.name.toLowerCase() === name.toLowerCase() && u.agentNumber === agentNumber);
...
export const registerNewUser = (name, agentNumber, role, leaderId = null) => {
  const users = JSON.parse(localStorage.getItem('users')) || [];
  if (users.find(u => u.agentNumber === agentNumber)) return { success: false, message: 'Nomor Agen sudah terdaftar!' };
  const newUser = { name, agentNumber, role, target: role === 'leader' ? 1000 : 100 };
...
export const updateBanner = (id, data) => { const b = getBanners(); const i = b.findIndex(x => x.id === id); if (i !== -1) { b[i] = { ...b[i], ...data }; localStorage.setItem('banners', JSON.stringify(b)); } };
...
// --- FUNGSI BARU: OTOMATIS MENYELESAIKAN TUGAS BERDASARKAN JUDUL ---
export const completeTaskByTitle = (titleKeyword, agentNumber) => {
  const t = getTasks();
  let changed = false;
  t.forEach(task => {
    if (task.agentNumber === agentNumber && task.title.includes(titleKeyword) && task.status === 'Pending') {
...
// --- FUNGSI DATABASE SALES TRACKING ---
export const getProspects = () => JSON.parse(localStorage.getItem('prospects')) || [];
export const addProspectDB = (prospect) => { 
  const p = getProspects(); 
  p.unshift({ 
    ...prospect, id: Date.now(), status: 'Contact', 
    appointmentDate: null, appointmentTime: [], 
    followUpCount: 0, followUpLogs: [], closingLogs: [] 
  }); 
  localStorage.setItem('prospects', JSON.stringify(p)); 
};

export const setAppointmentDB = (id, date, timeSlots) => {
  const p = getProspects();
  const index = p.findIndex(x => x.id === id);
  if (index !== -1) {
    p[index].status = 'Appointment';
    p[index].appointmentDate = date;
    p[index].appointmentTime = timeSlots || [];
    localStorage.setItem('prospects', JSON.stringify(p));
  }
};

export const submitFollowUpDB = (id, note) => {
  const p = getProspects();
  const index = p.findIndex(x => x.id === id);
  if (index !== -1) {
    p[index].status = 'Follow Up';
    p[index].followUpCount = (p[index].followUpCount || 0) + 1;
    if (!p[index].followUpLogs) p[index].followUpLogs = [];
    
    p[index].followUpLogs.unshift({
      count: p[index].followUpCount,
      date: new Date().toISOString(),
      apptDate: p[index].appointmentDate,
      apptTime: p[index].appointmentTime,
      note: note
    });
    localStorage.setItem('prospects', JSON.stringify(p));
  }
...
export const submitClosingDB = (id, product, amount, agentNumber, leaderId) => {
  const p = getProspects();
  const index = p.findIndex(x => x.id === id);
  if (index !== -1) {
    p[index].status = 'Closed';
    if (!p[index].closingLogs) p[index].closingLogs = [];
    
    p[index].closingLogs.unshift({
      id: Date.now(),
      date: new Date().toISOString(),
      product: product,
    ...
    addPremiumInput(agentNumber, leaderId, p[index].name, product, amount);
  }
};

export const incrementProspectFollowUpDB = (id) => {
  const p = getProspects();
  const index = p.findIndex(x => x.id === id);
  if (index !== -1) {
    p[index].followUpCount = (p[index].followUpCount || 0) + 1;
...
export const updateProspectStatusDB = (id, newStatus, appointmentDate = null) => {
  const p = getProspects();
  const index = p.findIndex(x => x.id === id);
  if (index !== -1) {
    p[index].status = newStatus;
    if (appointmentDate) p[index].appointmentDate = appointmentDate;
    localStorage.setItem('prospects', JSON.stringify(p));
  }
};